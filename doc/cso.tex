...
\begin{itemize}
  ...
  \item Maybe: don't care alleles?
  \item We are mixing $\log$ and $\ln$
  \item Sell the whole thing as identifying the Pareto frontier
\end{itemize}
...
We are now able to formulate the probability given in \eqref{eq:single_prob} in
terms of the binary variables $h, b$ and $z$. For that, let $\xi_k^j$ denote
...
Since $(1-r_{p,q})\geq r_{p,q}$ for all $1\leq p<q<m$, we
may define $\Delta r_{p,q}:=(1-r_{p,q})-r_{p,q}$. We can express
$\Pr[]{\xi_k^j}$, assuming $x_{k,j}=1$, as follows.
\begin{align*}
\Pr[]{\xi_k^j} & = (1-\frac{h_j}{2})\prod_{p=1}^{m-1}\prod_{q=p+1}^m \bigg( (1-b_{j,p,q}) \\
               & \quad\quad\quad\quad\quad + b_{j,p,q}\cdot (r_{p,q}\cdot
               z_{k,p,q}+(1-r_{p,q})\cdot (1-z_{k,p,q}))   \bigg)\\
&=(1-\frac{h_j}{2})\prod_{p=1}^{m-1}\prod_{q=p+1}^m \left((1-b_{j,p,q}) + b_{j,p,q}\cdot (1-r_{p,q}-\Delta r_{p,q} z_{k,p,q})\right)
\end{align*}
Indeed, for a homozygous genotype $j$ all elements in the upper product evaluate
to 1 as desired. In the heterozygous case, every maximal homozygous block
contributes $r_{p,q}$ if it contains at least one crossover (first case in
\eqref{eq:single_prob}), and $(1-r_{p,q})$ otherwise (second case in
\eqref{eq:single_prob}). The corresponding log-probability of event $\xi_k^j$ is as
follows.
...
\begin{equation*}
\begin{split}
  \ln(\Pr[]{\xi_k^j}) &= h_j \ln(\frac{1}{2})+\sum_{p=1}^{m-1}\sum_{q=p+1}^m b_{j,p,q}\cdot \ln(1-r_{p,q}) \\
&\qquad\qquad + \sum_{p=1}^{m-1}\sum_{q=p+1}^m b_{j,p,q} \cdot z_{k,p,q}\cdot \ln(\frac{r_{p,q}}{1-r_{p,q}})
\end{split}
\end{equation*}

If $j_1$ and $j_2$ are the two parental genotypes of chromosomes $2i-1$
and $2i$ forming genotype $i$, we compute in variable $p_i$ the $\log$
probability of event $\xi^{j_1}_{2i-1}\cap\xi^{j_2}_{2i}$
as $\ln(\Pr[]{\xi^{j_1}_{2i-1}})+\ln(\Pr[]{\xi^{j_2}_{2i}})$. For that we have
to sum over all possible $1 \leq j<i$ to identify $j_1$ and $j_2$:
\begin{align}
 \label{eq:something}
   p_i &= \sum^{i-1}_{j=1}\sum_{k=2i-1}^{2i} x_{k,j} \cdot \ln(\Pr[]{\xi_k^j})\\
       & =\sum^{i-1}_{j=1}\sum_{k=2i-1}^{2i} x_{k,j}\bigg(h_j\ln(\frac{1}{2})+\sum_{p=1}^{m-1}\sum_{q=p+1}^m b_{j,p,q}\ln(1-r_{p,q})\\
   &\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad+ \sum_{p=1}^{m-1}\sum_{q=p+1}^m  b_{j,p,q} \cdot z_{k,p,q} \ln(\frac{r_{p,q}}{1-r_{p,q}}) \bigg)   
\end{align}
\obacht{eq differs from conference version, see comment in .tex, MEK: it's good}

\HL{Notice that in case genotype $i$ is heterozygous, its two chromosomes may
swap their originating genotypes as accounted for in the second
case of equation \eqref{eq:combined_prob}. We model such an event
$\xi^{j_2}_{2i-1}\cap\xi^{j_1}_{2i}$ with the shadow variables
$\tilde{x}_{k,i} \in \{0,1\}$ and the following constraints.
\begin{align}
  \tilde{x}_{2i-1,j} & \leq x_{2i,j} & n < i \leq L, 1 \leq j < i\\
  \tilde{x}_{2i,j} & \leq x_{2i-1,j} & n < i \leq L, 1 \leq j < i\\
  \sum_{j=1}^{i-1} \tilde{x}_{2i-1,j} & = \sum_{j=1}^{i-1} \tilde{x}_{2i,j} & n < i \leq L\\
  \tilde{x}_{k,j} & \leq h_{\delta(k)} & 2n < k \leq 2L, 1 \leq j < \delta(k)
\end{align}
In addition, we need to ensure that the swapped parental genotypes are
actually able to give rise to the two chromosomes. We do this by introducing
shadow variables $\tilde{y}_{k,p}$, $\tilde{g}_{k,p,l} \in \{0,1\}$ that have
similar meaning as their non-shadow counterparts as expressed by the following
constraints.
\begin{align}
  \tilde{g}_{k,p,2i}   - a_{2i,p}   \cdot \tilde{x}_{k,i} \cdot (1-\tilde{y}_{k,p}) & = 0 & 2n < k \leq 2L, 1 \leq p \leq m, 1 \leq i < \delta(k)\\
  \tilde{g}_{k,p,2i-1} - a_{2i-1,p} \cdot \tilde{x}_{k,i} \cdot \tilde{y}_{k,p}     & = 0 & 2n < k \leq 2L, 1 \leq p \leq m, 1 \leq i < \delta(k)\\
  \tilde{g}_{k,p,2i} + \tilde{g}_{k,p,2i-1} - a_{k,p} \cdot \tilde{x}_{k,i}         & = 0 & 2n < k \leq 2L, 1 \leq p \leq m, 1 \leq i < \delta(k)
\end{align}
Now we can express the log-probability $\tilde{p}_i$ of observing
$\xi^{j_2}_{2i-1}\cap\xi^{j_1}_{2i}$ as follows.
\begin{equation}
 \begin{split}
   \tilde{p}_i=\sum^{i-1}_{j=1}\sum_{k=2i-1}^{2i} \tilde{x}_{k,j}\bigg(h_j\ln(\frac{1}{2})&+\sum_{p=1}^{m-1}\sum_{q=p+1}^m b_{j,p,q}\ln(1-r_{p,q})\\
   &+ \sum_{p=1}^{m-1}\sum_{q=p+1}^m  b_{j,p,q} \cdot \tilde{z}_{k,p,q} \ln(\frac{r_{p,q}}{1-r_{p,q}}) \bigg)   
 \end{split}
\end{equation}}
...
More precisely, we re-express the population size w.r.t.~probability
$p:=\Pr[]{j_1,j_2\rightarrow i}$ as:
$$N(p,\gamma) = \frac{\ln(1-\gamma)}{\ln(1-(\mathrm e^{p_i} + \mathrm{e}^{\tilde{p}_i}))}$$
\obacht[mohammed]{Change the text, we use an extension of the $\lambda$-method to
...
\begin{align*}
  d_1 & = \log(1 - (1 - \gamma)^\frac{1}{N_\mathrm{max}})\\
  \ldots\\
  d_{\ell-1} & = \log(0.5) = -0.69\\
  d_{\ell}   & = \log(1)   = 0\\
  e_1 & = 0\\
  e_2 & = d_1\\
  e_{\ell-1} & = d_{\ell-2}\\
  e_{\ell} & = d_{\ell-1}
\end{align*}
The vertices of the triangulation are given by
\begin{align*}
  N_{j,k} & = N(\mathrm{e}^{d_j} + \mathrm{e}^{e_k}, \gamma) && 1 < j \leq \ell,
  1 < k \leq \ell\\
  N_{j,1} & = N(\mathrm{e}^{d_j}, \gamma) && 1 \leq j \leq \ell.
\end{align*}
