...
\begin{align}
 \label{eq:something}
   ...
   &\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad+ \sum_{p=1}^{m-1}\sum_{q=p+1}^m  b_{j,p,q} \cdot \tilde{z}_{k,p,q} \ln(\frac{r_{p,q}}{1-r_{p,q}}) \bigg)   
\end{align}
...
\begin{align}
  ...
  \tilde{x}_{k,j} & \leq h_i & 2n < k \leq 2L, 1 \leq j < \delta(k)
\end{align}
...
\begin{equation}
 \begin{split}
   \tilde{p}_i=\sum^{i-1}_{j=1}\sum_{k=2i-1}^{2i} x_{k,j}\bigg(h_j\ln(\frac{1}{2})&+\sum_{p=1}^{m-1}\sum_{q=p+1}^m b_{j,p,q}\ln(1-r_{p,q})\\
