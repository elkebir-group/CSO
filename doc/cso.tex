...
% \begin{align}
%   \label{eq:ilp_x}
%   x_{k,j} \in \{0,1\} && 2n < k \leq 2L, j<\delta(k)
% \end{align}
...
\end{align}
% In the MIP, $L$ is an upper bound on the number of nodes in any feasible
% crossing schedule. Since there might be feasible schedules with less than $L$
% nodes, there can be chromosomes that do not have an originating genotype. That
% is why the previous constraints are inequality constraints. It must hold,
% however, that a chromosome has an originating genotype if and only if its
% counterpart has one as well:
% \begin{align}
%   \label{eq:ilp_x_2}
%   \sum_{j=1}^{i - 1} x_{2i-1,j} - \sum_{j=1}^{i - 1} x_{2i,j} = 0 && n < i\leq L
% \end{align}
...
as $\ln(\Pr[]{\xi^{j_1}_{k_1}})+\ln(\Pr[]{\xi^{j_2}_{k_2}})$. For that we have to sum over all possible $j<i$ to identify $j_1$ and $j_2$:
\begin{equation}
 \label{eq:something}
 \begin{split}
\bar{z}_i=\sum_{j<i}\sum_{l\in\{1,2\}}x_{k_l,j}\bigg(h_j\ln(\frac{1}{2})&+\sum_{p=1}^{m-1}\sum_{q=p+1}^m b_{j,p,q}\ln(1-r_{p,q})\\
 &+ \sum_{p=1}^{m-1}\sum_{q=p+1}^m  b_{j,p,q} \cdot z_{k_l,p,q} \ln(\frac{r_{p,q}}{1-r_{p,q}}) \bigg)   
 \end{split}
\end{equation}
\obacht{eq differs from conference version, see comment in .tex}

%This is what we had in the latest svn version:
%\bar{z}_i=\sum_{j<i}\sum_{l\in\{1,2\}}&x_{k_l,j}\bigg(h_j\ln(\frac{1}{2})+\sum_{p=1}^{m-1}\sum_{q=p+1}^m b_{j,p,q}\ln(1-r_{p,q})\\
%&+ \sum_{p=1}^{m-1}\sum_{q=p+1}^m \sum_{r=p+1}^q b_{j,p,q} \cdot  \ln(\frac{r_{p,q}}{1-r_{p,q}}) \cdot |y_{k,r} - y_{k,r-1}| \bigg)
