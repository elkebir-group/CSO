...
protected:
  ...
  void applyLinkageAnalysisUnlinked(const DpItemVector& newItemVector,
                                    const LociPairList& lociList,
                                    const int targetChromosome,
                                    DpItemSet& itemsToKeep) const;
  void applyLinkageAnalysisWeaklyLinked(const DpItemVector& newItemVector,
                                        const LociPairList& lociList,
                                        const int targetChromosome,
                                        DpItemSet& itemsToKeep) const;
  int applyInitialParentSelfing();
  virtual void cross(const Genotype& genotype1,
                     const Genotype& genotype2,
                     DpItemSet& newItemSet,
                     bool updateFlag);

public:
  HeuristicSolver(const Data* pData,
                  DpTable* pTable,
                  int maxIterations,
                  int verbosity,
                  HeuristicType heuristicType,
                  int nGenotypesToSelect,
                  int nGenotypesLinkageAnalysis,
