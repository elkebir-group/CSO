...
void generate(unsigned int seed, int nLoci, int nParents, int maxCorrectAlleles, double gamma,
	double costGen, double costCrossover, double costNode, int popMax,
	const Genotype& ideotype, const DoubleVector& mapDistances, double homozygousProb)
{
	...
	printf("<CSO nLoci=\"%d\" gamma=\"%f\" popMax=\"%d\" costCrossover=\"%lf\" costGen=\"%lf\" costNode=\"%lf\">\n",
...
int main(int argc, char **argv)
{
	...
	double costGen, costCrossover, costNode;
  int popMax;
	double gamma;
	...
	desc.add_options()
		...
		("cost-gen", po::value<double>(&costGen)->default_value(100), "Cost of a generation")
		("cost-node", po::value<double>(&costNode)->default_value(100), "Cost of an internal node (crossing)")
		("cost-crossover", po::value<double>(&costCrossover)->default_value(1), "Cost of schedule probability")
